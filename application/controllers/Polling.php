...
class Polling extends Frontend_Controller {
	...
	public function submit_polling(){
		$rules = $this->Polling_m->rules_save_choice_polling;
		$this->form_validation->set_rules($rules);
		$this->form_validation->set_message('required', 'Form %s tidak boleh dikosongkan');

		if ($this->form_validation->run() == TRUE) {
			$data = $this->Polling_m->array_from_post(array('idUSER','idPOLLING'));
            $data['nameCHOICE'] = $this->input->post($data['answer']);

            if(empty($data['idUSER']) OR empty($this->session->userdata('idUSER'))){
            	$data = array(
					'title' => 'Error!',
                    'text' => 'Maaf, anda diharuskan untuk login terlebih dahulu, atau jika anda ingin mendaftar silakan menuju ke link ini "'.base_url().'user'.'"'
                );
