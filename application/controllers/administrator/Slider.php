...
class Slider extends Admin_Controller {
	...
	public function saveslider() {
		...
		if ($this->form_validation->run() == TRUE) {
			$data = $this->Slider_m->array_from_post(array('titleSLIDER','descSLIDER'));
	        ...
	        if ($this->upload->do_upload('imgSLIDER')){
	        	$data['uploads'] = $this->upload->data();
	  			...
	  			redirect('administrator/slider/index_slider');
	        }
	...
	public function deleteimgslider($id1=NULL){
        ...
        $this->session->set_flashdata('message',$data);
		redirect('administrator/slider/index_slider/'.$id1);
	}
}